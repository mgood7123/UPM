...
//             regs.r9 - Stores the sixth argument
    ...
    const char * _syscall = 1;

    // <‎velco‎>‎ open/close/read/write/fcntl/access/chmod/opendir/readdir/closedir and several others

    // 0 will allow the specified syscall to be modified
    // 1 will prevent modification of the syscall
    // 2 will prevent read and modification of the syscall
...
//         printf ("SYSCALL %s() MODIFICATION IS NOT YET SUPPORTED from pid %d\n", syscallnum, pid);
    ...
    {
        printf ("SYSCALL %s() IS NOT YET SUPPORTED BY THIS APPLICATION from pid %d\n", syscallnum, pid);
    }
    char does_not_exist(const char * replacement)
    {
        printf("\n%s does not exist, aborting call modification\n", replacement);
    }
...
static int wait_for_syscall(pid_t child)
{
    ...
    while (1) {
        ...
        if (WIFSTOPPED(status)) {
            if (WSTOPSIG(status) != SIGTRAP && WSTOPSIG(status) != 0x80 && WSTOPSIG(status) != 0x85 /*0x85 (133) appears to be used by ptrace*/ ) {
