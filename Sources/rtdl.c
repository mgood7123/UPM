...
//    // printf("print save_to_variable from resolve() %s\n", save_to_variable);
    while (*cptr) {
        cptr++;
    }
...
int find_yourself(char *result, size_t size_of_result)
{
  ...
  result[0]=0;
//      readlink(findyourself_save_argv0);
...
main (int argc, char **argv)
{
  ...
  if(1 || strcmpb(argv[0],newpath)) { write(1,"  findyourself=\"",16); write(1,newpath,strlenb(newpath)); write(1,"\"\n",2); }
  char *dummy  = strdupb( newpath );
  char *dname = dirname( dummy );
  write(1,"  Directory=\"",13); write(1,dname,strlenb(dname)); write(1,"\"\n",2);
